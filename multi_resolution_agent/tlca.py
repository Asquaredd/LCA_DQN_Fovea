...
class TLCA:
    ...
    def __init__(self,patch_size,dictionary_size,threshold,
                 dictionary,num_patch,device):
    ...
    def update_dict(self,inputdata):
        ...
        self.dictionary += self.lr*torch.matmul(delta_s,self.a.t())
...
def run_lca(video_p,model,t_dict_update,frame_analyze, device):
...
def run_lca_test(video_p,model,t_dict_update,frame_analyze, device):
    ...
    a_last=model.a
    error=model.error(video_p)
    
    model.u=torch.zeros(model.dict_size,model.num_patch).to(device)
    model.a=torch.zeros(model.dict_size,model.num_patch).to(device)
    
    return  a_last,error.mean()
...
def train_model(model,input_size, input_v, device, t_dict_update=10):
    
    epoch = 10
    num_sample = input_v.size(0)
    ...
    for t in range(epoch):
        ...
        for j in order_sample:
            ...
            if torch.max(video_piece)==0:
                continue
            inputdata=(video_piece.reshape((input_size,1)).float())   
